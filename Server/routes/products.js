...
exports.findByCode = function(req, res){
    res.setHeader('Content-Type', 'application/json');
    db.query("SELECT * FROM Products p WHERE p.code = ?", [req.params.code], function(err, rows) {
        res.json(rows);
    });
};
...
exports.delete = function(req, res) {
    ...
    db.query("DELETE FROM Products WHERE code = ?", [req.params.code], function(err, rows) {
        if (err) {
            res.send(false);
            console.log(err);
        } else res.send(true);
